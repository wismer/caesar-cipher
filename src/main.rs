...
fn main() {
    let first_message = "that thing. did you get that thing i sent you.".to_string();
    let second_message = "System check identified 1 issue (0 silenced).
    ...
    let encrypted = cipher.encode(&first_message);
    ...
    let encrypted_reverse = reverse_cipher.encode(&second_message);
